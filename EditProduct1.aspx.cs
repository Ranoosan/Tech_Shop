...
public partial class EditProduct1 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            // Check if ProductID2 exists in the query string
            if (Request.QueryString["ProductID2"] != null)
            {
                int productId;
                // Try parsing ProductID2 and load product details
                if (int.TryParse(Request.QueryString["ProductID2"], out productId))
                {
    ...
    private void LoadProductDetails(int productId)
    {
        ...
        string query = "SELECT ProductName, Category, Brand, RAMCapacity, ROMCapacity, Price, Description, ImageUrl, QuantityInStock FROM Products2 WHERE ProductID2 = @ProductID2";

        using (SqlConnection conn = new SqlConnection(connectionString))
        {
            SqlCommand cmd = new SqlCommand(query, conn);
            cmd.Parameters.AddWithValue("@ProductID2", productId);
            conn.Open();
    ...
    {
        int productId = int.Parse(Request.QueryString["ProductID2"]);
        ...
        string connectionString = ConfigurationManager.ConnectionStrings["data"].ConnectionString;
        string query = "UPDATE Products2 SET ProductName = @ProductName, Category = @Category, Brand = @Brand, RAMCapacity = @RAMCapacity, ROMCapacity = @ROMCapacity, Price = @Price, Description = @Description, ImageUrl = @ImageUrl, QuantityInStock = @QuantityInStock WHERE ProductID2 = @ProductID2";
        ...
        string script = "alert('Order placed successfully! Redirecting to product page...'); window.location='ProductList2.aspx';";
